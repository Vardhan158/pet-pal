...
import { CheckCircle, XCircle, UserPlus, LogOut } from "lucide-react";
...
if (!document.getElementById("ad-poppins")) {
  ...
  s.innerHTML = `
    ...
    .ad-card:hover { transform: translateY(-5px); box-shadow: 0 16px 40px rgba(99,102,241,0.12); }
    .ad-card-img { width: 100%; height: 180px; object-fit: cover; display: block; transition: transform 0.45s ease; }
    ...
    .ad-approve-btn {
      ...
      cursor: pointer; border: 1px solid #BBF7D0; transition: all 0.2s;
    }
    ...
    .ad-submit-btn {
      ...
      cursor: pointer; box-shadow: 0 4px 16px rgba(99,102,241,0.3);
    ...
    .ad-logout-btn:hover { background: #FEE2E2; box-shadow: 0 3px 10px rgba(220,38,38,0.15); }
    ...
    .ad-bottom-btn {
      ...
      font-size: 0.55rem; font-weight: 500; color: #9CA3AF;
      cursor: pointer; transition: color 0.15s; padding: 6px 2px;
      white-space: nowrap;
    }
    .ad-bottom-btn.active { color: #6366F1; }
    .ad-bottom-icon {
    ...
    .ad-bottom-btn.active .ad-bottom-icon { background: #EEF2FF; }
  `;
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axiosInstance.get(endpoint, { headers: { Authorization: `Bearer ${token}` } });
      setData(
        res.data.pendingPets || res.data.approvedPets ||
        res.data.rejectedPets || res.data.sellers || res.data.users || []
      );
    } catch (err) {
  ...
  const handleReview = async (id, action) => {
    try {
      await axiosInstance.put(`/admin/review/${id}`, { action }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleAddSeller = async (e) => {
    e.preventDefault();
    try {
      await axiosInstance.post("/admin/create-seller", sellerForm, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const pageContent = (
    <main style={{
      flex: 1,
      overflowY: "auto",
      padding: isMobile ? "1rem 1rem 80px" : "40px 36px",
    }}>
      {/* Page header */}
      <motion.div
        initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.45 }} style={{ marginBottom: 32 }}
      >
        <div style={{
          display: "inline-flex", alignItems: "center", gap: 8,
          background: "#EEF2FF", borderRadius: 999,
          padding: "4px 16px", marginBottom: 12, border: "1px solid #C7D2FE",
        }}>
        ...
        <h1 style={{ fontSize: "clamp(1.3rem, 3vw, 2rem)", fontWeight: 800,
      ...
      <AnimatePresence mode="wait">
        ...
        {view === "addSeller" && (
          ...
          >
            <div style={{
              background: "#fff", borderRadius: 22, border: "1px solid rgba(0,0,0,0.05)",
              boxShadow: "0 4px 24px rgba(99,102,241,0.08)", padding: "32px 28px",
            }}>
        ...
        {view !== "offers" && view !== "addSeller" && (
          <motion.div key={view} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            {loading ? (
              ...
              >
                {data.map((item) => (
                  <motion.div key={item._id} variants={cardVariant} transition={{ duration: 0.38 }} className="ad-card">
                    {item.image && (
                    ...
                    <div style={{ padding: "16px 18px 18px", flex: 1, display: "flex", flexDirection: "column" }}>
                      ...
                      {item.price && (
                        <p style={{ margin: "0 0 14px", fontSize: "0.9rem", fontWeight: 700, color: "#6366F1" }}>
                          ₹{item.price}
                        </p>
                      )}
                      {view !== "pending" && (
      ...
      <div className="ad-root" style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: "0.95rem", color: "#0F172A", lineHeight: 1.2 }}>PetWorld</div>
              <div style={{ fontSize: "0.62rem", color: "#9CA3AF", lineHeight: 1.2 }}>Admin Panel</div>
            </div>
          </div>
          ...
          {mobileNavItems.map((item) => (
            <button
              ...
              style={{
                ...
                color: view === item.key ? "#6366F1" : "#9CA3AF",
  ...
  return (
    <div className="ad-root" style={{
      ...
      <AdminSidebar setView={setView} currentView={view} onLogout={handleLogout} />
      {pageContent}
    </div>
  );
}